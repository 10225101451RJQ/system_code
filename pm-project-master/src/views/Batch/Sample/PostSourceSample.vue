<template>
  <div>
    <h3>{{ $t('Sample.PostSourceSample.title') }}</h3>
    <el-form ref="sampleForm" :model="sampleInfo" :rules="rules" label-width="100px">
      <el-row>
        ...
        <el-col :span=4>
          <el-form-item :label="$t('Sample.common.sampleProperty')">
            {{ $t('Sample.PostSourceSample.sampleProperty') }}
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item :label="$t('Sample.common.sampleType.label')" prop="sampleType">
        <el-select v-model="sampleInfo.sampleType" :placeholder="$t('Sample.common.sampleType.rule')" clearable
                   @change="resolveBug">
          <el-option
              v-for="item in sampleInfo.sourceList"
              :key="item.value"
              :label="item.label"
              :value="item.value">
          </el-option>
      ...
      <el-row>
        ...
        <el-col :span=6>
          <el-form-item :label="$t('Sample.common.params.Vf.label')" :style="{display: stateShow}"
                        label-width="130px" prop="Vf">
            <el-input v-model="sampleInfo.Vf" :placeholder="$t('Sample.common.params.Vf.rule')"
                      style="width: 80%"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span=6>
          <el-form-item :label="$t('Sample.common.params.m.label')" :style="{display: stateShow}"
                        label-width="130px" prop="m">
  ...
  data() {
    ...
    return {
      ...
      sampleInfo: {
        ...
        batchName: "",
        sampleState: "",
        sampleProperty: this.$t('Sample.PostSourceSample.sampleProperty'),
        sampleType: "",
        sourceList: [],
        Vf: "", Df: "", m: ""
      },
      fileList: [],
      rules: {
        sampleType: [{required: true, message: this.$t('Sample.common.sampleType.rule'), trigger: "blur"}],
    ...
    async submitUpload() {
      ...
      await this.$refs.sampleForm.validate((valid) => {
        if (valid) {
          ...
          postFormData('/sample/postSourceSample', uploadData).then((resp) => {
            loading.close();
            if (resp.data.code === 0) {
