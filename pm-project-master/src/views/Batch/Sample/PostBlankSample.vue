<template>
  <div>
    <h3>{{ $t('Sample.PostBlankSample.title') }}</h3>
      ...
      <el-row>
        ...
        <el-col :span=4>
          <el-form-item :label="$t('Sample.common.sampleProperty')">
            {{ $t('Sample.PostBlankSample.sampleProperty') }}
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span=6>
          <el-form-item :label="$t('Sample.common.params.Df.label')" :style="{display: liquidShow}"
                        label-width="100px" prop="Df">
            <el-input v-model="sampleInfo.Df" :placeholder="$t('Sample.common.params.Df.rule')"
                      style="width: 80%"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span=6>
          <el-form-item :label="$t('Sample.common.params.Vf.label')" :style="{display: solidShow}" label-width="130px"
                        prop="Vf">
        ...
        <el-col :span=6>
          <el-form-item :label="$t('Sample.common.params.m.label')" :style="{display: solidShow}" label-width="130px"
                        prop="m">
  ...
  data() {
    ...
    return {
      ...
      sampleInfo: {
        ...
        batchName: "",
        sampleState: "",
        sampleProperty: this.$t('Sample.PostBlankSample.sampleProperty'),
        Vf: "", Df: "", m: ""
      },
      fileList: [],
      rules: {
    ...
    'sampleInfo.sampleState': {
      handler() {
        if (this.sampleInfo.sampleState === '固体' ||  this.sampleInfo.sampleState === 'solid') {
          this.solidShow = ''
          this.liquidShow = 'none'
          this.sampleInfo.Df = '-1'
    ...
    async submitUpload() {
      ...
      await this.$refs.sampleForm.validate((valid) => {
        if (valid) {
          ...
          postFormData('/sample/postBlankSample', uploadData).then((resp) => {
            loading.close();
            if (resp.data.code === 0) {
